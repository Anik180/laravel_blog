...
<div id="post-content">
   <div class="module fpad bmargin posted-by-snippet">
      <div class="col-xs-8 col-sm-9 btn-sm nohpad nobpad">
         Posted
         <span class="inline-block">
         in 
         <a class="bold" title="Event - Webinars" href="/events?category[]=Webinars">
   ...
   <div class="row">
      <div class="col-md-12 vmargin">
         <h1 class="bold h2">{{$singlepost->title}}</h1>
         <div class="clearfix"></div>
         <div class="clearfix"></div>
      </div>
   </div>
   <div class="alert-secondary btn-block text-center" style="margin-bottom:15px">
      <img class="center-block" alt="Agile and DevOps for the Oracle Cloud" title="Agile and DevOps for the Oracle Cloud" src="{{ url('storage/post/'.$singlepost->image) }}" />
      <div class="clearfix"></div>
   </div>
   <div class="clearfix"></div>
   <div class="row">
      ...
      <div class="well text-center bmargin">
         <div class="row">
            <div class="col-sm-6 xs-bmargin">
               <b> <i class="fa fa-calendar"></i>Created at</b> <br>
               {{ \Carbon\Carbon::parse($singlepost->created_at)->diffForhumans() }} 
            </div>
            <div class="col-sm-6">
               <b> <i class="fa fa-calendar"></i>Updated at</b> <br>
               {{ \Carbon\Carbon::parse($singlepost->updated_at)->diffForhumans() }} 
            </div>
         </div>
      </div>
  ...
  @endphp 
   <div class="event_files">
      <table class="table">
         <thead>
         	@foreach($tag as $row)
            <th><a href="{{ route('tag.post',$row->name)}}">{{$row->name}}</a></th>
            @endforeach
         </thead>
      </table>
   </div>
...
<div class="col-md-4">
        ...
        <div class="comments-details">
          <span class="total-comments comments-sort">{{$singlepost->comments()->count() }}Comments</span>
     
        </div>
        @guest
         <p>For post a new comment.You need to login first
            <button class="btn btn-danger"><a href="{{route('login')}}"></a></button>
         </p>
        ...
        <div class="comment-box add-comment">
          <span class="commenter-pic">
            <img src="{{asset('frontend/images/user-icon.jpg')}}" class="img-fluid">
          </span>
          <span class="commenter-name">
            <input type="text" placeholder="Add a public comment" name="comment">
            <button type="submit" class="btn" style="background-color: #4169e1;">Comment</button>
        ...
        @foreach($singlepost->comments as $row)
        <div class="comment-box">
          <span class="commenter-pic">
            <img src="{{asset('frontend/images/user-icon.jpg')}}" class="img-fluid">
          </span>
          <span class="commenter-name">
            <a href="#">{{ $row->user->name}}</a> <span class="comment-time">{{ $row->created_at->diffForHumans()}}</span>
          </span>       
