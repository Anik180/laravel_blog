...
<br>
<div class="container">
   <h1 ><strong>upcoming event</strong></h1>
   <hr>
   @if($events->count() > 0)
   ...
   <div class="search_result row-fluid member-level-4">
      <div class="grid_element">
         ...
         <div class="img_section col-sm-4 nopad sm-bmargin">
            <a title="{{$row->title}}" href="{{ route('single.events',$row->slug)}}">
         ...
         <div class="mid_section xs-nopad col-sm-8 ">
            <a class="h3 bold bmargin center-block" title="{{$row->title}}" href="{{ route('single.events',$row->slug)}}">{{$row->title}}</a>
            <div class="clearfix"></div>
            <div class="clearfix"></div>
            <p class="bpad xs-nomargin xs-nopad tmargin">{!! \Illuminate\Support\Str::limit($row->description,'150') !!}
               <a class="inline-block bold" title="{{$row->title}}" href="{{ route('single.events',$row->slug)}}"> View More </a> 
            </p>
            <div class="clearfix"></div>
            <div class="hidden-xs row-fluid bpad">
               <a title="{{$row->title}}" class="btn btn-success col-sm-5 view-details rmargin" href="{{ route('single.events',$row->slug)}}"> <i class="fa fa-search"></i> View Details</a>
               <div class="clearfix"></div>
            </div>
         </div>
      </div>
