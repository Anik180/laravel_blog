...
@section('content')
    <div class="container-fluid">
        <div class="block-header">
            <a class="btn btn-primary waves-effect" href="{{ route('admin.sliders.create') }}">
                            ...
                            <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                ...
                                <tbody>
                                    @foreach($slider as $key=>$row)
                                        <tr>
                                            <td><img src="{{URL::to($row->slider)}}" style="height: 80px; width: 180px;"></td>

                                            <td class="text-center">
                                                <a href="{{ route('admin.sliders.edit',$row->id) }}" class="btn btn-info waves-effect">
                                                    <i class="material-icons">edit</i>
                                                </a>
                                                <button class="btn btn-danger waves-effect" type="button" onclick="deleteSlider({{ $row->id }})">
                                                    <i class="material-icons">delete</i>
                                                </button>
                                                <form id="delete-form-{{ $row->id }}" action="{{ route('admin.sliders.destroy',$row->id) }}" method="POST" style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
